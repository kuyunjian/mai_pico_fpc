...
Q: 原神是什么？
A: 这是唯一需要自行理解的词，以及在任何社区，群聊请不要直接提起原神内容
...
Q：还是不会怎么办
A：优先看下面第七条**互补教学**，然后进群里或者评论区提问，提问请①附带前因后果②附带对焦清楚，截图清晰的图片or视频③提问的智慧.pdf
...
### <2>.3d部分打印
3. 按键帽（button）和装饰框（cover）使用树脂打印
树脂光滑而且强度足够
...
7. 如果你想用petg打印键帽，请使用群文件我优化后的模型kouer优化button.stl
8. 并且请向打印的人说明均竖着打印避免层纹，请使用群猫娘给出的**MAI.3mf**文件![[Pasted image 20250211141640.png]]
```
拓大了钢轴孔以适配petg层纹
缩短了按键大小以适配petg层纹（不然会卡键）
```
9. 如果你使用了源按键帽文件打印petg材料，请削以下一条边，以及割以下一条边 #拓展
### <3>.淘宝
1. 电子元器件在**淘宝**买，而不是拼多多
2. 请前往群内群文件下载**采购清单.txt**
3. **ito玻璃**请前往对应的店家对客服说**一份maip玻璃**，随后他会给出链接拍下即可
4. 各pcb之间的连接线缆实测**最低30awg**够用，**26awg**也可以
5. **电烙铁**建议买带开关的，并且不要买成外热烙铁头
6. [不要用送的焊锡](https://www.bilibili.com/video/BV1uh411F7KW/ )
7. **亚克力**请找深圳裕隆橡胶（精华消息）两套40元包邮 或者**简川旗舰店**（更便宜）` 亚克力每次请自己上传最近下载到的front.dwg , mid.dwg文件避免旧版本文件打印出来应发争议`（上方ito玻璃可以直接使用他的文件，不用自己上传文件~~而且不能ito玻璃属于批量定制，上传了也没用~~）
8. **外壳の铝型材**：github上imfera的图片没更新，以383.5mm两根252mm一根为准，去嘉立创领取20元fa商城新人券然后凑点螺丝可以仅邮费下单
9. 显示器有两种方案，①是买瑕疵**成品屏幕**（强烈推荐），②是买**液晶，屏幕排线，驱动板两块，喇叭**，液晶需要带挂耳的。如果没带挂耳，需要用3d转接件，液晶需要用imfrea的背板（有八个1mm孔）固定
10. 树莓派建议采购绿色的国产microusb接口树莓派，而不是黑色的粉色的树莓派，首先便宜只要10元左右``其次黑色的树莓派本人把个坏了三个，每个买来插上使用的时候，凑近了听，树莓派都有滋滋的电流声音。有一个树莓派有个输出口不知为何坏了，另一个树莓派直接寄了``![[Pasted image 20250303125959.png]]
11. 请给树莓派配套好一点的usb数据线，确保能传输数据（可以买的时候问商家是不是仅充电，或者）
12. 新人有个加热板就差不多得了，更别说热风枪和焊台或者回流焊台了
### <4>.嘉立创
嘉立创优惠券盘点
## Ⅱ. 焊接和测试
### <0>.前言
...
（一定要戴劳保手套）
![[7d564a4ddecc67b8d3fa5e71be783fcf.jpeg]]![[a342d844b630ae3505dccd2290d86b4f.jpeg]]
...
```
![[Pasted image 20250228224336.png]]
##### 1.2. 焊接教学
...
一大堆引脚焊油配合好点的焊锡丝一拖就好
例如树莓派的焊盘，mpr模块的引脚
![[bb9d7d0e6503bce2ea8e56fbf268441a.jpg]]
...
1. 功能上来说
6p焊接好了即可和主板通信
主要是5pin：vcc(在这个项目里用3v3，也就是3.3v直流电)，gnd（地，形成回路），scl，sda（iic或者叫i2c，时钟线），add（分配地址）
另外还有1pin：iRQ，在这个项目里面并没有产生作用，如果想了解这个pin的高低电位逻辑，请查阅技术手册

vcc电源和gnd地不做介绍
scl，sda时钟线用来和树莓派交换信息，让树莓派知道触摸了哪个引脚
add用来告诉树莓派，这是哪个mpr芯片

2. 拓展模块走线结构上来说：
scl，sda，irq分别接上了一个10kohm的电阻和vcc串联（scl，sda，irq是并联关系）
add默认和gnd通过一条走线连接在了一起，这是第一个地址（0x5A）通过切割这条走线，并且让add和vcc，scl，sda连接总共可以分配四个地址，也就是一个i2c总线可以搭配四个mpr使用，而这个项目中仅使用了前三个
